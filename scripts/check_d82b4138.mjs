...
for (const r of relays) {
  const evs = await query(r, { kinds: [30078, 30081], '#t': ['bro-order'], limit: 500 });
  for (const ev of evs) {
    try {
...
console.log('Full IDs:', [...fullIds]);

for (const fullId of fullIds) {
  console.log(`\n########## ${fullId} ##########`);
  for (const r of relays) {
    const evs = await query(r, { kinds: [30078, 30079, 30080, 30081], '#r': [fullId], limit: 50 });
    console.log(`\n--- ${r} (${evs.length}) ---`);
    for (const ev of evs.sort((a, b) => a.created_at - b.created_at)) {
      ...
      console.log(
        ...
        'd=' + (dTag?.slice(0, 45)),
        parsed?.providerInvoice ? 'HAS_INV' : ''
      );
    }
  }
}
