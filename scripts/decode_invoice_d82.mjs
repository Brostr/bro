...
import { decode } from 'file:///C:/Users/produ/Documents/GitHub/bro_app/backend/node_modules/light-bolt11-decoder/bolt11.js';
...
for (const ev of evs) {
  ...
  try {
    ...
    console.log('  expiry (seconds):', expiry, `(${(expiry / 86400).toFixed(1)} days)`);
    console.log('  expiresAt:', new Date(expiresAt * 1000).toISOString());
    console.log('  expired?', now > expiresAt, 'now:', new Date(now * 1000).toISOString());
    console.log('  amount (msat):', amount, '=>', amount / 1000, 'sats');
    console.log('  paymentHash:', paymentHash);
    console.log('  payee:', dec.payeeNodeKey?.slice(0, 20));
  } catch (e) {
